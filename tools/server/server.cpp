                            ...
                            if (slot.n_past > 0 && slot.n_past < (int) slot.prompt_state.tokens.size()) {
                                ...
                                if (pos_min > pos_min_thold) {
                                    ...
                                    if (do_reset) {
                                        ...
                                        slot.n_past = 0;
                                        slot.prompt_state.checkpoints.clear();
                                    }
                                }
                            }
            ...
            for (auto & slot : slots) {
                ...
                if (slot.state == SLOT_STATE_DONE_PROMPT) {
                    ...
                    if (llama_model_n_swa(model) > 0 && !params_base.swa_full && params_base.n_swa_checkpoints > 0) {
                        if (slot.prompt_state.checkpoints.size() >= (size_t) params_base.n_swa_checkpoints) {
                            {
                                const auto & cur = slot.prompt_state.checkpoints.back();
                        ...
                        for (const auto & checkpoint : slot.prompt_state.checkpoints) {
                        ...
                        SLT_WRN(slot, "SWA checkpoint create, pos_min = %d, pos_max = %d, size = %.3f MiB, total = %d/%d (%.3f MiB)\n",
                                cur.pos_min, cur.pos_max, (float) cur.data.size() / 1024 / 1024, (int) slot.prompt_state.checkpoints.size(), params_base.n_swa_checkpoints, size_total);
                    }
